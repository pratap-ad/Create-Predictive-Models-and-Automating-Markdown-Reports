...
## Required packages and their calls

The list of packages are: tidyverse, knitr, caret, gbm, rattle gbm, rmarkdown.
...
```{r}
#filter the data
popDayData<- popData  %>% filter(weekday_is_monday == params$value) %>% select(-is_weekend, -1)
#data table
DT::datatable(popData  %>% filter(weekday_is_monday == params$value))
```
```{r}  
...
```{r}
sumryData<- popDataFinalTrain %>% select(num_imgs, num_videos, global_subjectivity, global_rate_positive_words, shares)
summarise(sumryData)
