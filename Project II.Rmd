---
title: "Project II ST558"
author: "Pratap Adhikari"
date: "10/9/2020"
output: rmarkdown::github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, message = F, warning = F)
library(tidyverse)
library(haven)
library(knitr)
library(class)
library(caret)
```


```{r}
popData<- read_csv("OnlineNewsPopularity.csv")
popData

```

```{r}
#day<- function(day, ...){}

```


### Processing

```{r}
# checking if any missing values in the raw data
anyNA(popData)
```
No missing values were found.


```{r}
# Create train and test data set.
set.seed(2)
train<- sample(1:nrow(popData), size = nrow(popData) *0.7)
test<- dplyr::setdiff(1:nrow(popData), train)

popDataTrain<- popData[train, -1]
popDataTest<- popData[test,-1]
```


### Trai the model 

```{r}
trCtrl<- trainControl(method="repeatedcv", number=10, repeats=3)
knnFit<- train(shares ~., data=popDataTrain, method="knn",
               trControl=trCtrl,
               preProcess=c("center", "scale"),
               tunelength=10)

knnFit
```


### Plot of k vs Accuracy

The best number value for neighbors given by the model fit is `r knnFit$bestTune`.


```{r}
kacc<- tbl_df(knnFit$results[, 1:2])
ggplot(kacc, aes(x=k, y=RMSE)) + geom_point( color="blue") +
  geom_line(color="green") +
  ggtitle(" k vs. Accuracy")
```



```{r}

#Test the test data set by predicting it
test_pred<- predict(knnFit, newdata = popDataTest)
test_pred
```


```{r}
#Capture the predictions
pred<- predict(knnFit, newdata=popDataTest)
postResample(pred, obs=popDataTest$shares)<- tbl1$table


```


```{r}
#Find the prediction error rate 
misClass<- 1-sum(diag(tbl2))/sum(tbl2)
misClass

```
The miss classification percentage we got from out train model is `r round(misClass*100,2)` %















